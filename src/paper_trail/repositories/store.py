...
def _candidate(row: sqlite3.Row) -> PolicyCandidate:
...
def _link(row: sqlite3.Row) -> EvidenceLink:
    return EvidenceLink(
        id=row["id"],
        commitment_id=row["commitment_id"],
...
def _budget(row: sqlite3.Row) -> BudgetRecord:
    return BudgetRecord(
        id=row["id"],
        evidence_id=row["evidence_id"],
...
class PolicyRepository:
    ...
    def documents(self) -> list[SourceDocument]:
        rows = self._c.execute("SELECT * FROM documents ORDER BY id").fetchall()
        return [
    ...
    def candidates(self, status: ReviewStatus | None = None) -> list[PolicyCandidate]:
        if status:
            ...
            rows = self._c.execute("SELECT * FROM candidates ORDER BY source_page").fetchall()
    ...
    def set_candidate_status(self, candidate_id: int, status: ReviewStatus) -> None:
        self._c.execute("UPDATE candidates SET review_status=? WHERE id=?",
    ...
    def commitments(self, kind: CandidateType | None = None) -> list[Commitment]:
        if kind:
            rows = self._c.execute(
                "SELECT * FROM commitments WHERE kind=? ORDER BY code, id", (kind.value,)
            ).fetchall()
        else:
            rows = self._c.execute("SELECT * FROM commitments ORDER BY code, id").fetchall()
        return [_commitment(r) for r in rows]

    def commitment(self, commitment_id: int) -> Commitment | None:
        row = self._c.execute("SELECT * FROM commitments WHERE id=?", (commitment_id,)).fetchone()
...
class EvidenceRepository:
    ...
    def evidence(self, evidence_id: int) -> EvidenceItem | None:
        row = self._c.execute("SELECT * FROM evidence WHERE id=?", (evidence_id,)).fetchone()
    ...
    def links(self, status: ReviewStatus | None = None) -> list[EvidenceLink]:
        if status:
            ...
            rows = self._c.execute("SELECT * FROM links ORDER BY score DESC").fetchall()
    ...
    def link(self, link_id: int) -> EvidenceLink | None:
        row = self._c.execute("SELECT * FROM links WHERE id=?", (link_id,)).fetchone()
    ...
    def decide_link(self, link_id: int, status: ReviewStatus,
        ...
        self._c.execute(
            ...
            (status.value, relationship.value if relationship else None, link_id))
        self._c.commit()
    ...
    def budgets_for_evidence(self, evidence_id: int) -> list[BudgetRecord]:
        rows = self._c.execute(
            "SELECT * FROM budgets WHERE evidence_id=? ORDER BY id", (evidence_id,)
        ).fetchall()
